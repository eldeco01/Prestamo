...
namespace EASystem
{
	public partial class frmZona : Form
	{
        ...
        public frmZona(frmPrincipal mform)
		{
			InitializeComponent();
			mfrm = mform;
            ctrlInicio(false);
        ...
        private void btnSalvar_Click(object sender, EventArgs e)
        {
            ...
            int updResul = zonaDal.update(pObjet);
            if (updResul > 0)
            {
            ...
            {
                int resultado = zonaDal.insertar(pObjet);
                if (resultado > 0)
                {
        ...
        private void rNew()
        {
            txtId.Text = nextNum.buscar("zona").ToString();
        ...
        private void cargarDgv()
        {
            ...
            dgvContenido.DataSource = zonaDal.buscar();
        ...
        private void btnEliminar_Click(object sender, EventArgs e)
        {
            ...
            if (result == DialogResult.Yes)
            {
                ...
                pObjet.id = int.Parse(txtId.Text.Trim());
                pObjet.nom = txtNombre.Text.Trim();
                int delResul = zonaDal.delete(pObjet);
                if (delResul > 0)
                {
        ...
        {
            Tipos obj = new Tipos();
            obj = zonaDal.obtenerReg((int)dgvContenido.CurrentRow.Cells["ID"].Value);
