...
namespace EASystem
{
	public partial class frmTitel : Form
	{
		private frmPrincipal mfrm;
        private Boolean modSalvar = false;
        public frmTitel(frmPrincipal mform)
		{
			InitializeComponent();
			mfrm = mform;
            ctrlInicio(false);
        ...
        {
            Tipos pTitel = new Tipos();
            pTitel.id = int.Parse(txtId.Text.Trim());
            pTitel.nom = txtNombre.Text.Trim();
            int updResul = tiTelDala.updTitel(pTitel);
            if (updResul > 0)
            {
            ...
            {
                int resultado = tiTelDala.addTitel(pTitel);
                if (resultado > 0)
                {
                ...
                {
                    MessageBox.Show("No se pudo guardar el TipoTel", "Fallo!!", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                }
            } 
        }
        ...
        private void rNew()
        {
            txtId.Text = nextNum.buscar("titel").ToString();
        ...
        private void cargarDgv()
        {
            ...
            dgvContenido.DataSource = tiTelDala.buscar();
        ...
        {
            Tipos modTel = new Tipos();
            modTel = tiTelDala.ObtenerTel((int)dgvContenido.CurrentRow.Cells["ID"].Value);
            lblEstado.Text = "<<<Consultando>>>";
            lblEstado.ForeColor = Color.FromArgb(65, 105, 225);
            txtId.Text = modTel.id.ToString();
            txtNombre.Text = modTel.nom.ToString();
            ctrlInicio(true);
